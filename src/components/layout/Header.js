        ...
        <>
            <header className="px-6 py-4 mx-auto max-w-6xl">
                <nav className="flex justify-between items-center md:flex-nowrap flex-wrap">
                    <a href="#">
                        <Image src={"/assets/logo/oso.png"} alt="logo" width={56} height={80} className='w-10 sm:w-14'/>
                    </a>
                    <div className="hidden sm:flex items-center gap-6">
                        {!showMessage && (
                            <form onSubmit={form?.handleSubmit} className='flex gap-2'>
                                ...
                                <button type={hform ? "submit" : "button"} className="!py-[11px] !px-[23.25px] !text-base !w-auto theme-btn" onClick={() => setHForm(true)}>Join Waitlist</button>
                            </form>
                        )}
                        ...
                        {!!showMessage && (
                            <p className="text-gray-800 text-base">Congrats! You will soon be among the first to experience the power of OSO.</p>
                        )}
