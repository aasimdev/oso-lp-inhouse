...
const Video = () => {
    ...
    return (
        <div className="mt-8 md:mt-6 mx-auto max-w-[902px] rounded-lg md:rounded-2xl overflow-hidden text-center h-video relative">
            ...
            <video preload="none" muted autoPlay loop controls={!handler} className={`aspect-auto object-cover transition-all duration-300 ${preLoad ? "opacity-100" : "opacity-0"}`} loading="lazy" ref={videoRef}>
                <source src={'/assets/video/search.mp4'} type="video/mp4" />
            </video>

            {handler &&
                <button className={`w-36 h-36 sm:w-[200px] sm:h-[200px] rounded-full bg-purple bg-opacity-20 flex justify-center items-center group absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 transition-all duration-300 ${preLoad ? "opacity-100" : "opacity-0"}`} onClick={videoHandler}>
